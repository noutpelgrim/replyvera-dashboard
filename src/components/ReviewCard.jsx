...
const ReviewCard = ({ review, onApprove, onRegenerate }) => {
  ...
  const reviewId = review.google_review_id || '';
  const isFacebook = reviewId.startsWith('facebook-');
  const isTrustpilot = reviewId.startsWith('trustpilot-');
  const isScanned = reviewId.startsWith('scanned-');
  const isGoogle = reviewId && !isFacebook && !isTrustpilot && !isScanned;
  ...
  const renderPlatformIcon = () => {
    if (isFacebook) {
      return (
        <div style={{ width: '40px', height: '40px', background: '#1877F2', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
    ...
    if (isTrustpilot) {
      return (
        <div style={{ width: '40px', height: '40px', background: '#00B67A', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: '1.2rem', color: 'white' }}>⭐</span>
        </div>
      );
    }
  ...
  const getPlatformBadge = () => {
    if (isFacebook) return { text: 'FACEBOOK REVIEW', bg: 'rgba(24, 119, 242, 0.1)', color: '#1877F2', border: 'rgba(24, 119, 242, 0.2)' };
    if (isTrustpilot) return { text: 'TRUSTPILOT REVIEW', bg: 'rgba(0, 182, 122, 0.1)', color: '#00B67A', border: 'rgba(0, 182, 122, 0.2)' };
    if (isGoogle) return { text: 'GOOGLE REVIEW', bg: 'rgba(66, 133, 244, 0.1)', color: '#4285F4', border: 'rgba(66, 133, 244, 0.2)' };
    return { text: 'SCANNED LEAD', bg: 'rgba(108, 71, 255, 0.1)', color: 'hsl(var(--primary))', border: 'rgba(108, 71, 255, 0.2)' };
  };
